    ...
    const autoBetManager = (() => {
        ...
        function ensureStyles() {
            ...
            style.textContent = `
                ...
                .blaze-login-card .mode-card-title {
                    font-size: 15px;
                    font-weight: 700;
                    color: #ffffff;
                    margin-bottom: 4px;
                }
                .blaze-login-card .mode-card-subtitle {
                    margin: 0 0 16px 0;
                    font-size: 13px;
                    color: #9ca3af;
                }
...
async function persistAnalyzerState(newState) {
    ...
    function createSidebar() {
            ...
            <div class="auto-bet-modal" id="autoBetModal" style="display:none;">
                ...
                <div class="auto-bet-modal-content">
                    ...
                    <div class="auto-bet-modal-body">
                        <div class="blaze-login-card" id="blazeLoginCard">
                            <div class="mode-card-title">Conta Blaze</div>
                            <p class="mode-card-subtitle">Conecte-se para usar o modo real</p>
                            <div class="blaze-login-status" id="blazeLoginStatus">
                                <div class="blaze-login-form" id="blazeLoginForm">
                                    <div class="auto-bet-field">
                                        <span>Email</span>
                                        <input type="email" id="blazeEmail" placeholder="seu@email.com" autocomplete="username" />
                                    </div>
                                    <div class="auto-bet-field">
                                        <span>Senha</span>
                                        <div class="password-input-wrapper">
                                            <input type="password" id="blazePassword" placeholder="••••••••" autocomplete="current-password" />
                                            <button type="button" class="password-toggle-btn" id="blazePasswordToggle" aria-label="Mostrar/Ocultar senha">
                                ...
                                <div class="blaze-login-success" id="blazeLoginSuccess" style="display: none;">
                                    <div class="success-icon">✓</div>
                                    <div class="success-info">
                                        <div class="success-title">Conectado</div>
                                        <div class="success-email" id="blazeConnectedEmail"></div>
                                    </div>
                                    <button type="button" class="blaze-disconnect-btn" id="blazeDisconnectBtn">
        ...
        const blazeLoginSuccess = document.getElementById('blazeLoginSuccess');
        const blazeEmailInput = document.getElementById('blazeEmail');
        const blazeConnectedEmail = document.getElementById('blazeConnectedEmail');
        const autoBetEnabledCheckbox = document.getElementById('autoBetEnabled');

        // Toggle mostrar/ocultar senha
        if (blazePasswordToggle && blazePasswordInput) {
            blazePasswordToggle.addEventListener('click', () => {
                const isPassword = blazePasswordInput.type === 'password';
                blazePasswordInput.type = isPassword ? 'text' : 'password';
                blazePasswordToggle.querySelector('.toggle-text').textContent = isPassword ? 'Ocultar' : 'Mostrar';
            });
        }

        // Função para atualizar UI de conexão
        const updateBlazeConnectionUI = (connected, email = '') => {
            if (connected) {
                blazeLoginForm.style.display = 'none';
                blazeLoginSuccess.style.display = 'flex';
                blazeConnectedEmail.textContent = email;
                if (autoBetEnabledCheckbox) {
                    autoBetEnabledCheckbox.disabled = false;
                }
            } else {
                blazeLoginForm.style.display = 'flex';
                blazeLoginSuccess.style.display = 'none';
                if (autoBetEnabledCheckbox) {
                    autoBetEnabledCheckbox.disabled = true;
                    autoBetEnabledCheckbox.checked = false;
                }
            }
        };

        // Função para fazer login na Blaze
        const handleBlazeLogin = async () => {
            const email = blazeEmailInput.value.trim();
            const password = blazePasswordInput.value;
            ...
            try {
                ...
                if (data.success) {
                    ...
                    try {
                        ...
                        console.warn('[BlazeLogin] Erro ao salvar sessão:', e);
                    }

                    updateBlazeConnectionUI(true, email);
                    console.log('[BlazeLogin] ✅ Conectado com sucesso!');
                } else {
                    alert(`Erro ao conectar: ${data.error || 'Falha desconhecida'}`);
                    console.error('[BlazeLogin] ❌ Erro:', data.error);
                }
            } catch (error) {
                alert(`Erro ao conectar com o servidor: ${error.message}`);
                console.error('[BlazeLogin] ❌ Erro de rede:', error);
            } finally {
        ...
        const handleBlazeDisconnect = () => {
            blazeSessionData = null;
            try {
                localStorage.removeItem('blazeSession');
            } catch (e) {
                console.warn('[BlazeLogin] Erro ao remover sessão:', e);
            }
            updateBlazeConnectionUI(false);
            blazeEmailInput.value = '';
            blazePasswordInput.value = '';
            console.log('[BlazeLogin] 🔓 Desconectado');
        };

        // Verificar sessão salva ao abrir modal
        const checkSavedBlazeSession = () => {
            try {
                const saved = localStorage.getItem('blazeSession');
                if (saved) {
                    blazeSessionData = JSON.parse(saved);
                    updateBlazeConnectionUI(true, blazeSessionData.email);
                    console.log('[BlazeLogin] 🔄 Sessão restaurada');
                }
            } catch (e) {
                console.warn('[BlazeLogin] Erro ao restaurar sessão:', e);
                localStorage.removeItem('blazeSession');
            }
        };
        ...
        // Event listeners
        if (blazeLoginBtn) {
            blazeLoginBtn.addEventListener('click', handleBlazeLogin);
        }

        if (blazeDisconnectBtn) {
            blazeDisconnectBtn.addEventListener('click', handleBlazeDisconnect);
        }

        // Permitir Enter para fazer login
        if (blazePasswordInput) {
            blazePasswordInput.addEventListener('keypress', (e) => {
                if (e.key === 'Enter') {
                    handleBlazeLogin();
                }
            });
        ...
        if (autoBetConfigBtn) {
            ...
            openAutoBetModal = () => {
                checkSavedBlazeSession();
                originalOpenModal();
            };
            autoBetConfigBtn.removeEventListener('click', originalOpenModal);
            autoBetConfigBtn.addEventListener('click', openAutoBetModal);
        }
